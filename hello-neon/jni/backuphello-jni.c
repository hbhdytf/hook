...
//Android加载器通过linker中提供的接口dlopen，dlsym,dlerror和dlclose来动态加载和链接共享库
...
int ptrace_readdata( pid_t pid,  uint8_t *src, uint8_t *buf, size_t size )
{
	...
	for ( i = 0; i < j; i ++ )
	{
	// ptrace(PTRACE_PEEKTEXT, pid, src, data)
	//从内存地址中读取一个字节，pid表示被跟踪的子进程，内存地址由src给出，
...
int ptrace_attach( pid_t pid )
{
	...
	//	DEBUG_PRINT( "ptrace_attach  errno:%d",errno );
		DEBUG_PRINT( "ptrace_attach  errno" );
		return -1;
	}
	/*
	 if(errno == EPERM)
...
void* get_module_base( pid_t pid, const char* module_name )
{
	...
	if ( fp != NULL )
	{
		while ( fgets( line, sizeof(line), fp ) )
		{
			...
			{
				//strtok分解字符串为一组字符串,strtoul将字符串转换成无符号长整型数
				pch = strtok( line, "-" );
...
int inject_remote_process( pid_t target_pid, const char *library_path, const char *function_name, void *param, size_t param_size )
{
	...
	//pt_regs参数描述了在执行该系统调用时，用户态下的CPU寄存器在核心态的栈中的保存情况。
	//可执行文件路径的指针（regs.ebx）、命令行参数的指针（regs.ecx）和环境变量的指针（regs.edx）
                                                   ...
                                                   jobject thiz )
{
	...
	int success = inject_remote_process( target_pid, "/dev/tina/libhello.so", "hook_entry", "I'm parameter!", strlen("I'm parameter!") );
	...
	return (*env)->NewStringUTF(env, "successv3");
